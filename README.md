...
- proposes **`IRSTrade`** between **Alice and Bob** via the real
  ...
  through `Alice-USD`/`Bob-USD` cash accounts in **Bank**;
- proposes **`CDSTrade`** between **Alice and Bob** the same way,
  ...
  2031-07-20;
- proposes **`TRSTrade`** between **Alice and Bob** the same way,
...
The demo below drives all three seeded trades. 
The ledger/API ports it prints are: **6865** (gRPC Ledger API),
**7575** (JSON API), **7500** (Navigator).

## Demo

A guided walkthrough of all three products' full lifecycles, one section
per product: IRS's **daily margin in both directions**, a **periodic settlement**, then **novation** and **termination**; CDS's **premium settlement** 
and **credit event**; TRS's **daily variation margin**,
**periodic reset**, and an **index-level adjustment (rebasing)**. 

CDS and TRS both reuse the exact same novation/termination mechanics the IRS section
...
the `TRSTrade` all together (see "What `CashHolding` maps to" section). The
...
Keep the account and the live trade visible in Alice's and
...
Watch Alice's and Bob's windows update with new versions of `IRSTrade` and `CashHolding`. The recreated trade also carries
...
level**, a **periodic reset that trues up only the residual**, and an
**index-level adjustment** that rebases the index scale with no cash leg
at all.
...
### 4. Index-level adjustment (rebasing, no cash)

The index provider announces a rebasing of the published level — resetting
the base value/divisor, a methodology change, or similar (see
"What's intentionally simplified" below for why this is deliberately
**not** framed around single-stock corporate actions like a split or
special dividend — a properly-maintained index absorbs those on its own,
without ever touching the published level). In the **oracle** window, on
the current `TRSTrade`, exercise **`AdjustIndexLevel`**:

- `reason` = `IndexRebasing`
...
**not** touched by this adjustment — only `SettleMargin`/`SettleReset`
advance it, so the next VM's funding still counts elapsed days from
`2026-08-16` (the reset three steps up), not from this rebase.
...
- `asOf` = `2026-08-25` (9 days after the reset in step 3 — the
  adjustment above didn't move `lastSettledDate`, so this is still a
...
`testIndexAdjustmentNoPhantomLoss` for this guarded directly, and
`testIndexAdjustmentThenResetResidualCorrect` for confirming a
...
> rejected on `AdjustIndexLevel`, but there's deliberately no upper
> bound — a real rebase can be `×0.01` (the demo just used one).
>
...
- **Total-return index by construction, so no separate dividend or withholding-tax modeling.** `indexName`'s published level already
...
- **Simple, non-compounded daily funding accrual** — each `SettleMargin`/
  ...
  accrual period.
- **`AdjustIndexLevel` only models index-level events, deliberately not single-stock corporate actions.** `IndexAdjustmentReason`
  (`IndexRebasing`/`MethodologyChange`/`SuccessorIndex`/`DataCorrection`)
  covers things the index *provider* does to the published series
  itself — not a split, special dividend, or merger affecting one
  constituent, which a properly-maintained index absorbs via its own
  divisor without the published level ever jumping (that's the entire
  point of divisor maintenance). Even within that narrower, index-
  appropriate scope, every reason reduces to one
  `adjustmentFactor : Decimal` rebasing both index anchors — real
  divisor/methodology changes can be complex, this model treats them all
  as one multiplicative rescale. Heavier ISDA-adjacent outcomes —
  substituting the reference index entirely (`SubstituteReference`),
  reducing exposure and settling the cancelled portion
  (`PartialCancelAndPay`), or a calculation-agent-determined early
  close-out (`CancelAndPay`) — aren't modeled at all, not even as stubs;
  `AdjustIndexLevel`'s multiplicative rebase is the only index-event
  response this codebase has.
- **`AdjustIndexLevel` is controlled by `oracle`, not a separate Calculation Agent party.** Real ISDA practice distinguishes a party
  that reports objective market data from the Calculation Agent, who
  makes discretionary determinations like how to rebase for an index
  event — a real distinction this codebase collapses into one party for
...
- **`TRS.daml`** — the TRS-specific contract, same propose-accept shape
  ...
  - `AdjustIndexLevel` — a state-mutating oracle event with **no cash
    leg at all** (a shape none of IRS/CDS's oracle choices have): the
    oracle submits a multiplicative `adjustmentFactor` rebasing both
    `lastIndexLevel` and `lastResetIndexLevel` together, modeling an
    `IndexAdjustmentReason` like the provider rebasing the series or
    changing its methodology — deliberately not single-stock corporate
    actions (a split, special dividend, merger), which a well-maintained
    index absorbs via its own divisor without the published level ever
    jumping.
  - `NovationProposal` / `NovationConfirmation` / `TerminationProposal`
...
We use Daml **contract key** (`Cash.daml`'s
`CashAccountKey` = issuer + owner + currency — see the SDK's own
`daml-intro-3` tutorial for the pattern) to map specific `ContractId CashHolding` on every trade, refreshing it on each settle. `CashHolding` is identified
...
- **Not how a production token/holding actually works.** Real Canton
  ...
  teaching project; it demonstrates atomic DvP without
...
authorization chain traced precisely, step by step — including a
nested-box diagram of how each party's authority enters at one depth of
the transaction and falls through to the depths below it.
